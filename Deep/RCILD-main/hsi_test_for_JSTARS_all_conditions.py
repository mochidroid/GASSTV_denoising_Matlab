...
parser.add_argument("--model", type=str, default='model/complex.pth', help="model path")
parser.add_argument("--num_of_layers", type=int, default=15, help="Number of total layers")
parser.add_argument("--noise_case", type=str, default="i.i.d-G, Complex", help='i.i.d-G, n.i.i.d-G, Complex')
...
images_set = ['JasperRidge', 'PaviaU']
noise_conditions_set = ['g0.05_ps0.05_tint0', 
                        'g0.1_ps0.05_tint0', 
                        'g0.05_ps0_tint0.5', 
                        'g0.1_ps0_tint0.5', 
                        'g0.05_ps0.05_tint0.5',
                        'g0.1_ps0.05_tint0.5']

for image in images_set:
    for noise_condtion in noise_conditions_set:
        datapath = '../../../HSIData/' + image + '/clean_noisy_dataset/' + noise_condtion + '.mat'
        ...
        if __name__ == "__main__":
            ...
            cln_hsi = data['HSI_clean'].astype(np.float32)
            noi_hsi = data['HSI_noisy'].astype(np.float32)

            # GW
            std_e = get_variance(noi_hsi)
            #std_e = opt.std*np.ones(300)
            print(' GW variance :%.4f\n' % np.mean(std_e))
            noi_hsi_GW = GW(noi_hsi, std_e)
            cleanUV = RCDIP_test(model, noi_hsi_GW, opt.rank, opt.Ite)
            
            # IGW
            cleanUV = IGW(cleanUV, std_e)
            mpsnr, mssim, avsam1, ergas = msqia(cleanUV, cln_hsi)
            print(mpsnr, mssim)
            plt.imshow(cleanUV[:,:,50])

            SaveFolderName = './RCILD_result/' + image + '/' + noise_condtion
            os.makedirs(SaveFolderName, exist_ok=True)

            SaveFilename = SaveFolderName + '/image_result.mat'
            savemat(SaveFilename, {'HSI_clean': cln_hsi, 'HSI_noisy': noi_hsi, 'HSI_restored': cleanUV})
