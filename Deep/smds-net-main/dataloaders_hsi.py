...
import collections.abc as container_abcs  # 修正
string_classes = (str,)  # 修正
int_classes = int  # 修正
...
def load_mat_v73(file_name):
    ...
    with h5py.File(file_name, 'r') as f:
        data = f['rad'][:]  # 'DataCube'はファイル内の変数名に合わせてください
...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.grey:
            ...
            if img_name.endswith('.mat'):
                image = load_mat_v73(img_name)  # MATLAB v7.3ファイルを読み込む
            else:
                image = Image.open(img_name).convert('RGB')
                image = np.array(image).astype(np.float32)

            image = image / image.max()  # 正規化
