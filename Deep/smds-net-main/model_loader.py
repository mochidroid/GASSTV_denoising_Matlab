...
#             new_state_dict = OrderedDict()
#             for k, v in state_dict.items():
#                 if (k.startswith('apply_A') or k.startswith('apply_D') or k.startswith('apply_W')) and v.size() != model.state_dict()[k].size():
#                     print(f"Skipping loading parameter {k} due to size mismatch: {v.size()} vs {model.state_dict()[k].size()}")
#                     continue  # サイズが一致しない場合は読み込みをスキップ
#                 # name = k
#                 name = k[7:]  # remove 'module.' of dataparallel
#                 new_state_dict[name] = v
...
def load_model(model_name, model):
    print(f"Skipping ckpt loading for {model_name}, starting fresh training.")
