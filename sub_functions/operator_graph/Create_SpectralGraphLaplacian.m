function [L_delta, B, lam_max_Ldelta, info_l] = Create_SpectralGraphLaplacian(X, num_segments, sigma_l, k_lap, order_filt)
...
S      = num_segments;
k_lap  = max(0, min(k_lap, max(0,K-1)));
...
for s = 1:S
    ...
    if counts(s) > 0
        ...
        % r(s,:) = sum(reshape(tmp, [], n3), 1) / counts(s); %mean
        
        pixel_vals = reshape(X, [], n3);
        mask_vec = mask(:);
        r(s,:) = median(pixel_vals(mask_vec, :), 1); % median
...
% 【修正2】sigma計算用の距離行列で、画素数(counts)による重み付けを廃止
% Gmat = sqrt(Wseg) * Rdiff;  % <-- これがバグの原因（値が巨大になる）
Gmat = Rdiff;                 % 重みなしの生の値を使用
GTG  = Gmat.' * Gmat;         
nrm2 = diag(GTG);
...
if ischar(sigma_l) || isstring(sigma_l)
    switch lower(string(sigma_l))
        case "med"
            % 【修正3】D2_globalは「全セグメントの距離の総和」に近い性質を持つため
            % セグメント数 S で割って「平均的な距離」にする
            val_sigma_l = sqrt(median(double(d2_vec)) / S); 
            sigma_l_mode = "median";
        case "90"
