...
function [HSI_restored, removed_noise, output] ...
     = func_GASSTV_real_for_denoising_CondatVu(HSI_noisy, params)
fprintf('** Running func_GASSTV_real_for_denoising_CondatVu **\n');
...
lambda_sp = lambda_rho_sp * (norm(HSI_noisy(:), 1)/(n1*n2)) * sum(abs(Wsp.*Dsp(HSI_noisy)), "all");
