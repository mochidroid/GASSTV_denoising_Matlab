...
function [HSI_restored, removed_noise, other_result] = func_TPTV_for_denoising(HSI_clean, HSI_noisy, params, ~)

[M,N,p] = size(HSI_noisy);
sizeD   = size(HSI_noisy);
...
if (~isfield(params,'lr_init'))
    lr_init = 'SVD';
else
    lr_init = params.lr_init;
end

if (~isfield(params,'initial_rank'))
    r0 = 1;
else
    r0 = params.initial_rank;
end

if (~isfield(params,'Rank'))
...
% X              = lr_init;
X              = reshape(HSI_noisy,M*N,p);
E              = Y - X;
% X              = zeros([M*N,p], 'single');
% E              = zeros([M*N,p], 'single');
...
%              show3Dimg(reshape(W_s,M,N,3))
    ...
    %        show3Dimg(reshape(X,M,N,p))
            Xo(iter)=  norm(HSI_clean(:)-X(:),'fro')/norm(HSI_clean(:),'fro');
    %% -Update E
    ...
    move_mpsnr(iter) = calc_MPSNR(reshape(X, [M,N,p]), HSI_clean);
    move_mssim(iter) = calc_MSSIM(reshape(X, [M,N,p]), HSI_clean);
    %     load('Simu_indian.mat');
    %     [mp(iter),sm(iter),er(iter)]=msqia(simu_indian,reshape(X,[M,N,p]));
end

%% Organizing results for output
HSI_restored = reshape(X, [M, N, p]);
removed_noise.all_noise = reshape(E, [M, N, p]);

other_result.move_mpsnr = move_mpsnr;
other_result.move_mssim = move_mssim;
