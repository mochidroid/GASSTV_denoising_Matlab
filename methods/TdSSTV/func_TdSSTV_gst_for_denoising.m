%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U,S,T) = |D(Phi(U))|_1 + L1ball(S) + L1ball(T) + 
%               L2ball(U+S+T) + box constraint(U) + Dv(T)=0
%
% f1(U,S,T) = 0
% f2(U,S,T) = L2ball(U+S+T) 
% f3(U,S,T) = |D(Phi(U))|_1 + box constraint(U) + 
%               L1ball(S) + L1ball(T) + (Dv(T) = 0)
%
% A = (DPhi O O; I O O; O I O; O O I; O O Dv)
%
...
fprintf("** Running func_TdSSTV_gst_for_denoising **\n");
...
Phi     = @(z) dct_hsi_gpu(z, sizeof);
Phit    = @(z) idct_hsi_gpu(z, sizeof);
...
gamma1_U    = gpuArray(single(1/(8 + 1)));
...
switch L
    case "L1" % p = 1, prox of L1 norm
        Prox_Y1 = @(z) ProxDCweightedL1(z/gamma2, 1/gamma2, w);
    case "L12" % p =2, prox of mixed L1,2 norm
        Prox_Y1 = @(z) ProxDCweightedL12(z/gamma2, 1/gamma2, w);
end
...
for i = 1:maxiter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    U_tmp   = U - gamma1_U*(Phit(Dt(Y1)) + Y2);
    S_tmp   = S - gamma1_S*Y3;
    ...
    move_function_value(i) = sum(abs(D(Phi(U))), "all");
