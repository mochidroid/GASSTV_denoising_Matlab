%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U,S,T) = |WD(Phi(U))|_1 + L1ball(S) + L1ball(T) + 
%               L2ball(U+S+T) + box constraint(U) + Dv(T)=0
%
% f1(U,S,T) = 0
% f2(U,S,T) = L2ball(U+S+T) 
% f3(U,S,T) = |WD(Phi(U))|_1 + box constraint(U) + 
%               L1ball(S) + L1ball(T) + (Dv(T) = 0)
%
% A = (WDPhi O O; I O O; O I O; O O I; O O Dv)
%
...
fprintf("** Running func_GraphTdSSTV_gst_for_denoising **\n");
...
L           = params.L;
sigma_x     = gpuArray(single(params.sigma_x));
...
DPhiU = D(Phi(HSI_clean));
W = exp(-abs(DPhiU(:,:,1,:) / sigma_x));
...
for i = 1:maxiter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    U_tmp   = U - gamma1_U*(Phit(Dt(W.*Y1)) + Y2);
    S_tmp   = S - gamma1_S*Y3;
    ...
    % Updating Y1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Y1_tmp  = Y1 + gamma2*(W.*(D(Phi(U_res))));
    ...
    move_function_value(i) = sum(abs(W.*(D(Phi(U)))), "all");
