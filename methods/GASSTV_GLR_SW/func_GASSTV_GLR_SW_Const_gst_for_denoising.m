...
fprintf('** Running func_GASSTV_GLR_SW_Const_gst_for_denoising **\n');
...
opts_GLR.num_segments = single(params.num_segments);
opts_GLR.order_filt = single(params.order_filt);
...
[Wsp, rho_sp_est] = Create_SpatialGraphWeight(HSI_noisy, sigma_sp);

rho_sp = rho_sp_est * lambda_rho_sp;


[L_delta_cpu, B_cpu, lam_max_vec, info_l] = ...
    Create_GraphWeight_GLR_SW_Med(HSI_noisy, opts_GLR);
...
% Estimate radius of spatial graph TV

% rho_sp = sum(abs(Wsp.*Dsp(HSI_clean)), "all") * lambda_rho_sp;
...
for i = 1:maxiter
    ...
    Y2_next = Y2_tmp - gamma2*ProjFastL1Ball(Y2_tmp, rho_sp);
