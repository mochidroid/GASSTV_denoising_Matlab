...
%  ========================================================================
function [HSI_restored, removed_noise, other_result] = func_LRTDTV(~, HSI_noisy, params, ~)
% tic
sizeD           = size(HSI_noisy);

normD           = norm(HSI_noisy(:)); 
n               = prod(sizeD);
% maxIter         = 50;
maxIter         = 20000;
% epsilon         = 1e-4;
...
M1              = zeros(size(HSI_noisy)); % The multiplier for          
M2              = M1;
...
for iter = 1: maxIter
    ...
    temp       = 1/2*(HSI_noisy-S+Z+(M1-M2)/beta);
...
%     Z          = reshape(z,sizeD);
    ...
    %% - update S 
    S          = softthre(HSI_noisy-X+M1/beta,lambda/beta);% sparse
%     S          = beta*(Noi-X+M1/beta)/(beta+2*lambda);
    %% - update M
    M1         = M1 + beta*(HSI_noisy-X-S);
    M2         = M2 + beta*(X-Z); 
    %% - update Gamma
