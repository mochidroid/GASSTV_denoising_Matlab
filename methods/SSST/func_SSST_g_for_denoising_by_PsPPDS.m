%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U) = Σ_{k=1}^{K}|P_k(D(U))|_* + 
%           L2ball(U) + box constraint(U)
%
% f1(U) = 0
% f2(U) = box constraint(U)
% f3(U) = |P(D(U))|_*,N + L2ball(U)
%
% A = (PD; I)
%
...
function [HSI_restored, removed_noise, other_result] ...
     = func_SSST_g_for_denoising_by_PsPPDS(HSI_clean, HSI_noisy, params)
...
for i = 1:maxiter
    ...
    Y2_tmp  = Y2 + gamma2_Y2.*(2*U_next - U);    
    Y2_next = Y2_tmp - gamma2_Y2.*ProjL2ball(Y2_tmp./gamma2_Y2, HSI_noisy_gpu, epsilon);
