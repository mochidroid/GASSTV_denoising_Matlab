%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U,T) = Σ_{k=1}^{K}|P_k(D(U))|_* + 
%           L1ball(T) + 
%           L2ball(U+T) + box constraint(U) + Dv(T)=0
%
% f1(U,T) = 0
% f2(U,T) = box constraint(U) + L1ball(T)
% f3(U,T) = |P(D(U))|_*,N + L2ball(U+T) + Dv(T)=0
%
% A = (PD O; I I; O Dv)
%
...
function [HSI_restored, removed_noise, other_result] ...
     = func_SSST_gt_Cont_for_denoising_by_PsPPDS(HSI_clean, HSI_noisy, params)
...
load(tmp_save_name, ...
    'U_curr', 'T_curr', 'Y1_curr', 'Y2_curr', 'Y3_curr', 'i_curr', ...
    'converge_rate_U', 'converge_rate_T', 'converge_rate_N', ...
    'running_time', 'move_mpsnr', 'move_mssim', 'l2ball');
...
clear U_curr T_curr Y1_curr Y2_curr Y3_curr
...
for i = i_curr+1:maxiter
    ...
    Y2_tmp  = Y2 + gamma2_Y2.*(2*(U_next + T_next) - (U + T));  
    Y2_next = Y2_tmp - gamma2_Y2.*ProjL2ball(Y2_tmp./gamma2_Y2, HSI_noisy_gpu, epsilon);
...
removed_noise.gaussian_noise        = HSI_noisy - HSI_restored - removed_noise.stripe_noise;
