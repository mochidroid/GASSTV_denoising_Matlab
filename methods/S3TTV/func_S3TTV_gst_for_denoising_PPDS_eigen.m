%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U,S,T) = Σ_{k=1}^{K}|P_k(D(Db(U)))|_* + 
%           L1ball(S) + L1ball(T) + 
%           L2ball(U+S+T) + box constraint(U) + Dv(T)=0
%
% f1(U,S,T) = 0
% f2(U,S,T) = box constraint(U) + L1ball(S) + L1ball(T)
% f3(U,S,T) = |P(D(Db(U)))|_*,N + L2ball(U+S+T) + Dv(T)=0
%
% A = (PDDb O O; O O Dv; I I I)
%
% Algorithm is based on Pock's P-PDS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [HSI_restored, removed_noise, other_result] ...
     = func_S3TTV_gst_for_denoising_PPDS_eigen(HSI_clean, HSI_noisy, params)
fprintf('** Running func_S3TTV_gst_for_denoising_PPDS_eigen **\n');
...
% Y1 = zeros([n1, n2, n3, 2, blocksize(1)/shiftstep(1), blocksize(2)/shiftstep(2)], 'single', 'gpuArray');
...
for i = 1:maxiter
    ...
    % Updating Y1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Y1_tmp  = Y1 + gamma2_Y1.*(P(D(Ds(U_res))));
    Y1_next = Y1_tmp - gamma2_Y1.*Prox_S3TTV_eigen(Y1_tmp./gamma2_Y1, 1./gamma2_Y1);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Updating Y2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Y2_tmp  = Y2 + gamma2_Y2.*(U_res + S_res + T_res);    
    Y2_next = Y2_tmp - gamma2_Y2.*ProjL2ball(Y2_tmp./gamma2_Y2, HSI_noisy_gpu, epsilon);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Updating Y3
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Y3_next = Y3 + gamma2_Y3.*Dv(T_res);
