...
% f3(U,S,T) = |D(Ds(U))|_1 + L2ball(U+S+T) + Dv(T)=0
%
% A = (DDs O O; I I I; O O Dv)
%
% Algorithm is based on Pock's P-PDS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [HSI_restored, removed_noise, other_result] ...
     = func_RSSTV_gst_for_denoising_by_NsPPDS(HSI_clean, HSI_noisy, params)
fprintf('** Running func_RSSTV_gst_for_denoising_by_NsPPDS **\n');
...
converge_rate_W = zeros([1, maxiter], 'single');
...
for i = 1:maxiter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Updating Primal Variables except W
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Updating W
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    W_tmp   = W + gamma1_W*(opL(Y1));
    W_next  = Prox_l12norm_d4(W_tmp, gamma1_W);
    W_res   = 2*W_next - W;
    ...
    % Updating Y1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Y1_next  = Y1 + gamma2*(D(Dl(U_res))-opLadj(W_res));
    ...
    converge_rate_W(i) = norm(W_next(:) - W(:),2)/norm(W(:),2);
...
other_result.interpolation         = gather(W);
...
other_result.converge_rate_W        = gather(converge_rate_W(1:other_result.iteration));
