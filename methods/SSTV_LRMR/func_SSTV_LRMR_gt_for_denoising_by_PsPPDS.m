...
function [HSI_restored, removed_noise, other_result] ...
     = func_SSTV_LRMR_gt_for_denoising_by_PsPPDS(HSI_clean, HSI_noisy, params)
fprintf('** Running func_SSTV_LRMR_gt_for_denoising_by_PsPPDS **\n');
...
for i = 1:maxiter
    ...
    Y3_tmp  = Y3 + gamma2_Y3.*(2*(U_next + T_next) - (U + T));    
    Y3_next = Y3_tmp - gamma2_Y3.*ProjL2ball(Y3_tmp./gamma2_Y3, HSI_noisy_gpu, epsilon);
    ...
    if (mod(i, saverate) == 0)
        ...
        save(tmp_save_name, ...
            'U_curr', 'T_curr', 'Y1_curr', 'Y2_curr', 'Y3_curr', 'Y4_curr', 'i_curr', ...
