%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f(U,S,T) = D(Dl(U)) + omega*D(U) + L1ball(S) + L1ball(T) + 
%               L2ball(U+S+T) + box constraint(U) + Dv(T)=0
%
% f1(U,S,T) = 0
% f2(U,S,T) = L2ball(U+S+T) 
% f3(U,S,T) = D(Dl(U)) + omega*D(U) + box constraint(U) + 
%               L1ball(S) + L1ball(T) + (Dv(T) = 0)
%
% A = (DDl O O; omega*D O O; I O O; O I O; O O I; O O Dv)
%
...
fprintf("** Running func_HSSTV4_gst_for_denoising **\n");
