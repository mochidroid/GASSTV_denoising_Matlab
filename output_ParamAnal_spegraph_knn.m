...
GASSTV_CondatVu.k_lap    = [2, 5, 8, 10, 15, 20, 30];
...
for idx_image = idc_images
    ...
    knn_list = GASSTV_CondatVu.k_lap(:);  % x
    sigma_l_list = GASSTV_CondatVu.sigma_l(:);  % y

    % Z should be [numel(y) x numel(x)]
    vals_MPSNR = nan(numel(sigma_l_list), numel(knn_list));
    vals_MSSIM = nan(numel(sigma_l_list), numel(knn_list));
    ...
    for iy = 1:numel(sigma_l_list)          % y index
        for ix = 1:numel(knn_list)        % x index

            params = struct();
            params.lambda_rho_sp = GASSTV_CondatVu.lambda_rho_sp(1);
            params.lambda2       = GASSTV_CondatVu.lambda2;                 % x
    ...
    x = knn_list(:).';        % row vector for imagesc x
